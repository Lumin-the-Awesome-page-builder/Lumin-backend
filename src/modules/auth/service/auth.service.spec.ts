...
describe('AuthService tests', () => {
  ...
  it('Test login', async () => {
    //@ts-ignore
    service.bcrypt.compare = jest.fn(() => (new Promise(resolve => resolve(true))))
    //@ts-ignore
  ...
  describe('Test refresh', () => {
    ...
    it('Test not eq lastLogin', async () => {
      ...
      const err = await service.refresh({id: 1, lastLogin: 1241}).catch(err => {
        return err
      })
  ...
  it('Test signup', async () => {
    //@ts-ignore
    service.bcrypt.hash = jest.fn(() => (new Promise(resolve => resolve(true))))
    //@ts-ignore
    service.jwt.signUser = jest.fn(( ) => ["1", "2"])
